...
class Recommended extends React.Component{
	...
	handleSelect(eventKey) {
  	...
  	getServiceData(queryData,page){
  		getRequest('/movie-app/recommended-movies',queryData,(err,response)=>{
			if(err){
				this.setState({
					isError:true
				});
			} else {
				let recommendedMovies = response.data.result,
					recommendedSection = getSectionsData(recommendedMovies);
				this.setState({
					page: page,
					section: recommendedSection,
					isDataAvailable: true,
					lastPage: recommendedMovies.total_pages
				});
			}
		});
  	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if(nextProps.isSearch){
			...
			this.getSearchedMovieList(nextProps.searchedMovie,this.state.page)
		}
	}

	getSearchedMovieList(searchedMovie,page){
		let queryData ={
			movieName: searchedMovie
		};
		getRequest('/movie-app/recommendedSearch', queryData, (err,response)=>{
			if(err){
				this.setState({
					isError:true
				});
